...
using SFH.IT.Hljodrit.Web.Helpers;
...
namespace SFH.IT.Hljodrit.Web.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> ResendEmail([FromBody] LoginModel model)
        {
            ...
            return Ok(new { error_description = "Please confirm by clicking the link sent to you." });
        }
        // POST Account/Register
        [AllowAnonymous]
        [Route("Register")]
        ...
        {
            var path = WebHelper.GetApplicationPath(Request).ToString().Replace("/api", "");

            if (username == null || code == null)
            {
                path += "?error=Values missing";
            }
            else
            {
        ...
        private async Task SendEmail(ApplicationUser user, ApplicationUserManager userManager)
        {
            var host = WebHelper.GetApplicationHost(Request);
            string code = await userManager.GenerateEmailConfirmationTokenAsync(user.Id);
            var callbackUrl = WebHelper.GetApplicationPath(Request) + "/Account/ConfirmEmail?username=" + HttpUtility.UrlEncode(user.Email) + "&code=" + HttpUtility.UrlEncode(code);
            await userManager.SendEmailAsync(user.Id, "E-mail confirmation for " + host,
                "Hi, <br/><br/>" +
                "Please confirm your email address for " + host + " by clicking  <a href=\"" + callbackUrl + "\">here</a><br/><br/>" +
                "Thanks."
            );
        }
